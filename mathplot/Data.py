...
def load_data(PATH, split=0.1):
    ...
    if len(images) == 0 or len(masks) == 0:
        raise ValueError("No images or masks found. Check the dataset path.")
    ...
    train_x, valid_x = train_test_split(
            images,
            test_size=valid_size if valid_size > 0 else 1,  # Ensure non-zero value
            random_state=42
    )
    train_y, valid_y = train_test_split(
            masks,
            test_size=valid_size if valid_size > 0 else 1,
            random_state=42
    )

    train_x, test_x = train_test_split(
            images,
            test_size=test_size if test_size > 0 else 1,  # Ensure non-zero value
            random_state=42
    )
